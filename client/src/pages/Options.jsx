        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {items.map((item) => (
            <Link to={item.path} key={item.name} className="w-full">
              <div
                className="flex flex-col items-center justify-center w-full h-56 cursor-pointer bg-gray-800 rounded-lg border border-gray-700 transition-transform duration-300 ease-in-out transform hover:scale-105 hover:shadow-md hover:shadow-blue-600"
              >
                <div className="flex flex-col items-center justify-center h-full gap-4 p-6">
                  <span className="option-icon text-blue-400 text-5xl">{item.icon}</span>
